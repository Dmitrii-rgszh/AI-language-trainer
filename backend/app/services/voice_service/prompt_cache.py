...
WELCOME_REPLAY_STYLE = "coach"
WELCOME_PROOF_LESSON_MODEL_PHRASE = "I'd like a coffee without sugar."
...
def get_welcome_proof_lesson_model_audio_path(locale: str) -> Path:
    normalized_locale = "ru" if locale.lower().startswith("ru") else "en"
    base_dir = Path(settings.welcome_audio_cache_dir).resolve()
    return (base_dir / f"welcome_proof_lesson_model_{normalized_locale}.wav").resolve()
...
def ensure_welcome_proof_lesson_model_audio_cached(
    voice_service: VoiceService,
    *,
    locale: str,
) -> Path:
    normalized_locale = "ru" if locale.lower().startswith("ru") else "en"
    target_path = get_welcome_proof_lesson_model_audio_path(normalized_locale)
    if target_path.exists() and target_path.stat().st_size > 0:
        return target_path

    target_path.parent.mkdir(parents=True, exist_ok=True)
    audio_bytes = voice_service.synthesize(
        text=WELCOME_PROOF_LESSON_MODEL_PHRASE,
        language="en",
        speaker=WELCOME_REPLAY_SPEAKER,
        style=WELCOME_REPLAY_STYLE,
    )
