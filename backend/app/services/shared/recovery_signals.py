...
def build_resolution_map(
    ...
    for mistake in mistakes:
        ...
        resolution[title], _hint = resolve_resolution_status(linked_count, last_seen_days_ago)
...
def build_mistake_resolution(
    ...
    for mistake in ranked:
        ...
        status, hint = resolve_resolution_status(linked_vocabulary_count, last_seen_days_ago)
...
def resolve_resolution_status(linked_vocabulary_count: int, last_seen_days_ago: int) -> tuple[str, str]:
    if linked_vocabulary_count >= 2 and last_seen_days_ago >= 5:
        return (
            "stabilizing",
            "This weak spot has moved into vocabulary review and has not resurfaced recently.",
        )
    if linked_vocabulary_count >= 1 and last_seen_days_ago >= 2:
        return (
            "recovering",
            "The pattern is still being reviewed, but it is appearing less often in fresh corrections.",
        )
    return (
        "active",
        "This weak spot is still repeating often enough that it should stay in active recovery.",
    )
