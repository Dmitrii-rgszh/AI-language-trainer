...
def get_recommended_lesson(profile: UserProfile = Depends(require_profile)) -> Lesson:
    lesson = lesson_service.build_recommended_lesson(profile)
    if not lesson:
...
def start_lesson_run(
    payload: StartLessonRunRequest,
    profile: UserProfile = Depends(require_profile),
) -> LessonRunState:
    return lesson_runtime_service.start_run(profile, payload)


@router.get("/runs/active", response_model=LessonRunState | None)
def get_active_lesson_run(profile: UserProfile = Depends(require_profile)) -> LessonRunState | None:
    return lesson_runtime_service.get_active_run(profile)


@router.delete("/runs/{run_id}", status_code=204)
def discard_lesson_run(run_id: str, profile: UserProfile = Depends(require_profile)) -> None:
    lesson_runtime_service.discard_run(profile, run_id)


@router.post("/runs/{run_id}/restart", response_model=LessonRunState)
def restart_lesson_run(run_id: str, profile: UserProfile = Depends(require_profile)) -> LessonRunState:
    return lesson_runtime_service.restart_run(profile, run_id)
...
def submit_block_result(
    ...
    return lesson_runtime_service.submit_block_result(profile, run_id, payload)
...
def complete_lesson_run(
    ...
    return lesson_runtime_service.complete_run(profile, run_id, payload)
