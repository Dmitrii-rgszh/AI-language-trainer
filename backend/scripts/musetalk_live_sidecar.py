...
class PrepareAvatarRequest(BaseModel):
    ...
    image_path: str = Field(min_length=1, max_length=1200)
...
class MuseTalkLiveRuntime:
    ...
    def prepare_avatar(self, avatar_id: str, image_path: str) -> None:
        if avatar_id in self._avatars and self._avatars[avatar_id].image_path == image_path:
            return
        ...
        with self._avatar_lock:
            if avatar_id in self._avatars and self._avatars[avatar_id].image_path == image_path:
                return
            self._avatars[avatar_id] = self._build_avatar(avatar_id, image_path)

    def render_stream(self, avatar_id: str, audio_path: str, fps: int):
        ...
        if avatar_id not in self._avatars:
            ...
            self.prepare_avatar(avatar_id, default_image)
        ...
        def event_stream():
            ...
            try:
                for frame_index, frame in self._iter_avatar_frames(avatar, audio_path, runtime_fps):
    ...
    def _build_avatar(self, avatar_id: str, image_path: str) -> PreparedAvatar:
        ...
        coord_list_cycle = sanitized_coords + sanitized_coords[::-1]
    ...
    def _iter_avatar_frames(self, avatar: PreparedAvatar, audio_path: str, fps: int):
        ...
        for whisper_batch, latent_batch in generator:
            ...
            for res_frame in recon:
                yield frame_index, self._blend_frame(avatar, frame_index, res_frame)
                frame_index += 1
...
def prepare_avatar(payload: PrepareAvatarRequest) -> dict[str, Any]:
    try:
        runtime.prepare_avatar(payload.avatar_id, payload.image_path)
    except Exception as exc:
...
def render_stream(payload: RenderStreamRequest) -> StreamingResponse:
    try:
        iterator = runtime.render_stream(payload.avatar_id, payload.audio_path, payload.fps)
    except Exception as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
